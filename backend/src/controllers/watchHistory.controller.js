...
export const getCourseWatchHistoryController = async (req, res) => {
    try{
        ...
        const watchHistory = await getWatchHistoryByVideoIdAndUserId(videoId, "video", userId);
        if (!watchHistory) {
...
export const getMovieWatchHistoryController = async (req, res) => {
    try{
        const userId = req.user._id;
        const videoId = req.params.videoId;
        const watchHistory = await getWatchHistoryByVideoIdAndUserId(videoId, "movie", userId);
        if (!watchHistory) {
            return res.status(404).json({ message: "Watch history not found" });
        }
